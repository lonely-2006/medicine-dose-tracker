  ...
  return (
    <div className="auth-page">
      <div style={{ textAlign: 'center', position: 'relative', zIndex: 1 }}>
        <div style={{ fontFamily: 'Fraunces, serif', fontSize: 38, fontWeight: 700, color: '#fff', letterSpacing: '-1px' }}>Medi<span style={{ color: '#60a5fa' }}>Track</span></div>
        <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', letterSpacing: '2px', textTransform: 'uppercase', marginTop: 4 }}>Dose Tracker System</div>
      </div>
      <div className="auth-card">
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ fontSize: 13, color: '#94a3b8', marginTop: 4 }}>{mode === 'login' ? 'Sign in to your account' : 'Join MediTrack today'}</div>
        </div>
        <div style={{ display: 'flex', background: '#f1f5f9', borderRadius: 10, padding: 4, marginBottom: 24 }}>
          {['login', 'register'].map(m => (
            <button key={m} onClick={() => { setMode(m); setError(''); setSuccess('') }} style={{ flex: 1, padding: '9px 0', borderRadius: 8, border: 'none', background: mode === m ? '#fff' : 'transparent', color: mode === m ? '#0f172a' : '#94a3b8', fontWeight: mode === m ? 700 : 500, fontSize: 13, cursor: 'pointer', fontFamily: 'Plus Jakarta Sans, sans-serif', transition: 'all 0.2s', boxShadow: mode === m ? '0 1px 4px rgba(0,0,0,0.1)' : 'none' }}>{m === 'login' ? '🔑 Login' : '📝 Register'}</button>
          ))}
        </div>
        ...
        {mode === 'register' && (<div className="form-group"><label className="form-label">Full Name</label><input className="form-input" placeholder="Arjun Menon" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /></div>)}
        ...
        <div style={{ textAlign: 'center', marginTop: 18, fontSize: 12.5, color: '#94a3b8' }}>
          ...
          <span style={{ color: '#2563eb', cursor: 'pointer', fontWeight: 600 }} onClick={() => { setMode(mode === 'login' ? 'register' : 'login'); setError(''); setSuccess('') }}>{mode === 'login' ? 'Register here' : 'Login here'}</span>
        </div>
      </div>
      <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', position: 'relative', zIndex: 1 }}>Secured by Supabase Authentication</div>
    </div>
  )
}
...
function Sidebar({ pages, page, setPage, user, profile, onLogout }) {
  ...
  const roleColor = profile?.is_admin ? '#f59e0b' : profile?.is_doctor ? '#60a5fa' : '#34d399'
  ...
  return (
    <div>
      <div className="welcome-banner">
        <div>
          <div className="welcome-title">👑 Welcome, Admin {profile?.name?.split(' ')[0]}!</div>
          <div className="welcome-sub">Here is your system overview for today</div>
        </div>
        <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)', textAlign: 'right' }}>{new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</div>
      </div>
      <div className="stats-grid">
        {[
          { label: 'Total Users', value: stats.users, icon: '👥', accent: '#2563eb', accentLight: '#eff6ff' },
          { label: 'Doctors', value: stats.doctors, icon: '🩺', accent: '#0891b2', accentLight: '#ecfeff' },
          { label: 'Prescriptions', value: stats.prescriptions, icon: '📋', accent: '#d97706', accentLight: '#fffbeb' },
          { label: 'Medicines', value: stats.medicines, icon: '💊', accent: '#dc2626', accentLight: '#fef2f2' },
        ].map(c => (
      ...
      <div className="dash-grid">
        <div className="section-card">
          <div className="section-header">
            <div><div className="section-title">📅 Today's Schedule</div><div className="section-subtitle">Active medicines due today</div></div>
          ...
          {loading ? <Loader /> : scheds.length === 0 ? <Empty icon="📭" text="No schedules for today" /> :
            scheds.map(s => (
              <div key={s.schedule_id} className="schedule-item">
                ...
                <div className="schedule-dot" style={{ background: '#2563eb' }}></div>
                <div className="schedule-info">
                  <div className="schedule-name">{s.dosage?.medicine?.name || '—'}</div>
                  <div className="schedule-detail">{s.dosage?.amount}{s.dosage?.unit} · {s.dosage?.frequency} · {s.dosage?.medicine?.brand}</div>
                </div>
              </div>
            ))}
        </div>
      ...
      <div className="section-card">
        ...
        {loading ? <Loader /> : logs.length === 0 ? <Empty icon="📭" text="No logs yet" /> :
          <table><thead><tr><th>Log ID</th><th>Schedule</th><th>Date</th><th>Time Taken</th><th>Status</th></tr></thead>
          <tbody>{logs.map(l => (
            <tr key={l.log_id}><td>#{l.log_id}</td><td>SCH-{l.schedule_id}</td><td>{l.date}</td><td>{l.time_taken || '—'}</td>
...
function AdminUsers({ showToast }) {
  ...
  return (
    <div>
      <div className="section-card">
        <div className="section-header">
          <div><div className="section-title">👥 All Users</div><div className="section-subtitle">Manage all registered users</div></div>
      ...
      <div className="section-card">
        ...
        {loading ? <Loader /> : data.length === 0 ? <Empty icon="🩺" text="No doctors found" /> :
          <table>
            ...
            <tbody>{data.map(d => (
              <tr key={d.doctor_id}><td>#{d.doctor_id}</td><td><strong>{d.name}</strong></td>
              <td><span className="badge badge-blue">{d.specialization}</span></td>
              <td>{d.email}</td><td>{d.phone}</td>
              <td><button className="btn btn-danger btn-sm" onClick={() => remove(d.doctor_id)}>🗑️</button></td></tr>
            ))}</tbody>
          </table>}
      </div>
      {modal && (
      ...
      <div className="section-card">
        ...
        {loading ? <Loader /> : data.length === 0 ? <Empty icon="📋" text="No prescriptions" /> :
          <table>
            ...
            <tbody>{data.map(p => (
              <tr key={p.prescription_id}><td>#{p.prescription_id}</td><td><strong>{p.users?.name || '—'}</strong></td>
              ...
              <td><button className="btn btn-danger btn-sm" onClick={() => remove(p.prescription_id)}>🗑️</button></td></tr>
            ))}</tbody>
          </table>}
      </div>
      {modal && (
      ...
      <div className="section-card">
        ...
        {loading ? <Loader /> : data.length === 0 ? <Empty icon="💊" text="No medicines" /> :
          <table>
            ...
            <tbody>{data.map(m => (
              <tr key={m.medicine_id}><td>#{m.medicine_id}</td><td><strong>{m.name}</strong></td><td>{m.brand}</td>
              <td><span className="badge badge-green">{m.type}</span></td><td>PRE-{m.prescription_id}</td>
              <td><button className="btn btn-danger btn-sm" onClick={() => remove(m.medicine_id)}>🗑️</button></td></tr>
            ))}</tbody>
          </table>}
      </div>
      {modal && (
    ...
    <div className="section-card">
      ...
      {loading ? <Loader /> : data.length === 0 ? <Empty icon="🔔" text="No reminders" /> :
        <table>
          ...
          <tbody>{data.map(r => (
            <tr key={r.reminder_id}><td>#{r.reminder_id}</td><td>SCH-{r.schedule_id}</td>
            ...
            <td><button className="btn btn-ghost btn-sm" onClick={() => toggle(r.reminder_id, r.status)}>{r.status === 'Active' ? '🔕 Deactivate' : '🔔 Activate'}</button></td></tr>
          ))}</tbody>
        </table>}
    </div>
  )
...
function DoctorDashboard({ showToast, user, profile }) {
  ...
  return (
    <div>
      <div className="welcome-banner">
        <div>
          <div className="welcome-title">🩺 Welcome, Dr. {profile?.name?.split(' ')[0]}!</div>
          <div className="welcome-sub">Here is your patient overview for today</div>
        </div>
      ...
      <div className="stats-grid" style={{ gridTemplateColumns: 'repeat(2,1fr)' }}>
        {[
          { label: 'My Prescriptions', value: stats.prescriptions, icon: '📋', accent: '#2563eb', accentLight: '#eff6ff' },
          { label: 'Medicines Prescribed', value: stats.medicines, icon: '💊', accent: '#059669', accentLight: '#ecfdf5' },
        ].map(c => (
          <div key={c.label} className="stat-card" style={{ '--accent': c.accent, '--accent-light': c.accentLight }}>
            <div className="stat-icon">{c.icon}</div>
            <div className="stat-value">{loading ? '—' : c.value}</div>
      ...
      <div className="section-card">
        <div className="section-header">
          <div><div className="section-title">📋 Recent Prescriptions</div><div className="section-subtitle">Your latest prescriptions</div></div>
          <button className="btn btn-ghost btn-sm" onClick={load}>↻ Refresh</button>
        </div>
        {loading ? <Loader /> : prescriptions.length === 0 ? <Empty icon="📋" text="No prescriptions yet" /> :
          <table>
            ...
            <tbody>{prescriptions.map(p => (
              <tr key={p.prescription_id}><td>#{p.prescription_id}</td><td><strong>{p.users?.name || '—'}</strong></td>
              <td>{p.users?.email || '—'}</td><td>{p.date_issued}</td>
              <td style={{ maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.notes || '—'}</td></tr>
            ))}</tbody>
          </table>}
      </div>
    </div>
...
function DoctorPrescriptions({ showToast, user }) {
  ...
  return (
    <div>
      <div className="section-card">
        <div className="section-header">
          <div><div className="section-title">📋 My Prescriptions</div><div className="section-subtitle">Prescriptions you have issued</div></div>
          <button className="btn btn-primary btn-sm" onClick={() => setModal(true)}>+ Add</button>
        </div>
        {loading ? <Loader /> : data.length === 0 ? <Empty icon="📋" text="No prescriptions" /> :
          <table>
            <thead><tr><th>ID</th><th>Patient</th><th>Date</th><th>Notes</th></tr></thead>
            <tbody>{data.map(p => (
              <tr key={p.prescription_id}><td>#{p.prescription_id}</td><td><strong>{p.users?.name || '—'}</strong></td>
              <td>{p.date_issued}</td><td style={{ maxWidth: 220, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.notes || '—'}</td></tr>
            ))}</tbody>
          </table>}
      </div>
      {modal && (
  ...
  return (
    <div className="section-card">
      ...
      {loading ? <Loader /> : data.length === 0 ? <Empty icon="💊" text="No medicines prescribed" /> :
        <table>
          <thead><tr><th>Name</th><th>Brand</th><th>Type</th><th>Description</th><th>Prescription</th></tr></thead>
          <tbody>{data.map(m => (
            <tr key={m.medicine_id}><td><strong>{m.name}</strong></td><td>{m.brand}</td>
            ...
            <td>PRE-{m.prescription_id}</td></tr>
          ))}</tbody>
        </table>}
    </div>
  )
...
function DoctorSchedules({ showToast, user }) {
  ...
  return (
    <div className="section-card">
      <div className="section-header"><div><div className="section-title">📅 Patient Schedules</div><div className="section-subtitle">Medicine schedules for your patients</div></div></div>
...
function UserDashboard({ showToast, profile }) {
  ...
  const load = useCallback(async () => {
    ...
    setStats({ todayDoses: todayScheds.length, taken, missed, active: myScheds.length })
    setLoading(false)
  }, [profile, today])
  ...
  return (
    <div>
      <div className="welcome-banner">
        <div>
          <div className="welcome-title">👋 Hello, {profile?.name?.split(' ')[0]}!</div>
          <div className="welcome-sub">Track your medicines and stay healthy</div>
        </div>
      ...
      <div className="stats-grid">
        {[
          { label: "Today's Doses", value: stats.todayDoses, icon: '💊', accent: '#2563eb', accentLight: '#eff6ff' },
          { label: 'Taken', value: stats.taken, icon: '✅', accent: '#059669', accentLight: '#ecfdf5' },
          { label: 'Missed', value: stats.missed, icon: '⚠️', accent: '#dc2626', accentLight: '#fef2f2' },
          { label: 'Active Medicines', value: stats.active, icon: '🏥', accent: '#0891b2', accentLight: '#ecfeff' },
        ].map(c => (
          <div key={c.label} className="stat-card" style={{ '--accent': c.accent, '--accent-light': c.accentLight }}>
            <div className="stat-icon">{c.icon}</div>
      ...
      <div className="dash-grid">
        <div className="section-card">
          <div className="section-header">
            <div><div className="section-title">📅 Today's Schedule</div><div className="section-subtitle">Your medicines for today</div></div>
            <button className="btn btn-ghost btn-sm" onClick={load}>↻ Refresh</button>
          </div>
        ...
        <div className="section-card">
          ...
          {loading ? <Loader /> : reminders.length === 0 ? <Empty icon="🔕" text="No active reminders" /> :
            reminders.map(r => (
              <div key={r.reminder_id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '13px 20px', borderBottom: '1px solid #f1f5f9' }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 13.5, fontWeight: 700, color: '#0f172a' }}>SCH-{r.schedule_id}</div>
                  <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 2 }}>{r.mode} · {r.reminder_time}</div>
                </div>
...
function UserProfile({ showToast, profile, user }) {
  ...
  return (
    <div>
      <div style={{ background: 'linear-gradient(135deg, #0f1f3d, #1e3a6e)', borderRadius: 16, padding: '24px', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 20, color: 'white', boxShadow: '0 4px 20px rgba(15,31,61,0.2)' }}>
        <div style={{ width: 64, height: 64, borderRadius: '50%', background: 'rgba(96,165,250,0.2)', border: '2px solid rgba(96,165,250,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, fontWeight: 700, color: '#93c5fd', fontFamily: 'Fraunces, serif' }}>
          {getInitials(profile?.name)}
        </div>
        <div>
          <div style={{ fontFamily: 'Fraunces, serif', fontSize: 22, fontWeight: 700 }}>{profile?.name}</div>
          <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.55)', marginTop: 4 }}>{user?.email}</div>
          <span style={{ display: 'inline-block', marginTop: 8, background: 'rgba(52,211,153,0.2)', color: '#34d399', padding: '3px 10px', borderRadius: 20, fontSize: 11, fontWeight: 600 }}>Patient</span>
        </div>
      </div>
      <div className="section-card">
      ...
      {loading ? <Loader /> : data.length === 0 ? <Empty icon="🩺" text="No doctors assigned yet" /> :
        <table>
          ...
          <tbody>{data.map(d => (
            <tr key={d.doctor_id}><td>#{d.doctor_id}</td><td><strong>{d.name}</strong></td>
            <td><span className="badge badge-blue">{d.specialization}</span></td>
            <td>{d.email}</td><td>{d.phone}</td></tr>
          ))}</tbody>
        </table>}
    </div>
  )
      ...
      {loading ? <Loader /> : data.length === 0 ? <Empty icon="💊" text="No medicines prescribed" /> :
        <table>
          ...
          <tbody>{data.map(m => (
            <tr key={m.medicine_id}><td><strong>{m.name}</strong></td><td>{m.brand}</td>
            <td><span className="badge badge-green">{m.type}</span></td>
            <td style={{ maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{m.description || '—'}</td></tr>
          ))}</tbody>
        </table>}
    </div>
  )
      ...
      {loading ? <Loader /> : data.length === 0 ? <Empty icon="📅" text="No schedule found" /> :
        <table>
          ...
          <tbody>{data.map(s => {
            const active = s.start_date <= today && s.end_date >= today
            return (
              <tr key={s.schedule_id}><td><strong>{s.dosage?.medicine?.name || '—'}</strong></td>
              <td>{s.dosage?.amount} {s.dosage?.unit} · {s.dosage?.frequency}</td>
              <td>{s.start_date}</td><td>{s.end_date}</td>
              <td><span className="badge badge-blue">{s.time}</span></td>
              <td><span className={`badge ${active ? 'badge-green' : 'badge-red'}`}>{active ? 'Active' : 'Ended'}</span></td></tr>
      ...
      {loading ? <Loader /> : data.length === 0 ? <Empty icon="🔔" text="No reminders" /> :
        <table>
          ...
          <tbody>{data.map(r => (
            ...
            <td><span className="badge badge-blue">{r.mode}</span></td><td>{r.reminder_time}</td>
            <td><span className={`badge ${r.status === 'Active' ? 'badge-green' : 'badge-red'}`}>{r.status}</span></td>
            <td><button className="btn btn-ghost btn-sm" onClick={() => toggle(r.reminder_id, r.status)}>{r.status === 'Active' ? '🔕 Off' : '🔔 On'}</button></td></tr>
          ))}</tbody>
        </table>}
    </div>
  )
...
export default function App() {
  ...
  if (authLoading) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#0f1f3d', flexDirection: 'column', gap: 16 }}>
      <div style={{ fontFamily: 'Fraunces, serif', fontSize: 32, fontWeight: 700, color: '#fff' }}>Medi<span style={{ color: '#60a5fa' }}>Track</span></div>
      <div className="spinner" style={{ width: 28, height: 28, borderWidth: 3, borderTopColor: '#60a5fa', borderColor: 'rgba(255,255,255,0.2)' }}></div>
    </div>
  )
