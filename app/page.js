...
export default function AuthPage({ onLogin }) {
  ...
  const handleLogin = async () => {
    if (!form.email || !form.password) return setError('Enter email & password')
    setLoading(true)
    setError('')
  ...
  const handleRegister = async () => {
    if (role === 'admin') return setError('Admin accounts cannot self-register.')
    ...
    if (authData?.user) {
      // Step 2: Insert or upsert user profile
      ...
      if (role === 'doctor') {
        ...
        if (docErr) console.warn('Doctor insert failed:', docErr.message)
      }
    ...
    <div className="auth-page">
      ...
      <div className="role-selection">
        {roles.map(r => (
          <button
            key={r.id}
            style={{ background: r.id === role ? r.color : '#ccc' }}
      ...
      <p>
        ...
        <span style={{ textDecoration:'underline', cursor:'pointer' }} onClick={() => setMode(mode==='login'?'register':'login')}>
...
// ── USER MEDICINES ───────────────────────────────
export function UserMedicines({ showToast, profile }) {
  ...
  const load = useCallback(async () => {
    ...
    const { data: selfAdded } = await supabase
      .from('medicine')
  ...
  return (
    <div className="section-card">
      ...
      {loading ? <Loader /> :
        data.length === 0 ? <Empty icon="💊" text="No medicines yet — add one!" /> :
                ...
                <div key={m.medicine_id} className="medicine-card" style={{ opacity: m.status === 'Rejected' ? 0.6 : 1 }}>
                  ...
                  <div className="medicine-card-body">
                    ...
                    {!canSchedule && m.status === 'Pending' && (
                      <div style={{ marginTop: 10, fontSize: 12, color: '#d97706', background: '#fffbeb', borderRadius: 8, padding: '6px 10px' }}>
                        ⏳ Waiting for approval to add schedule
                      </div>
                    )}

                    {m.status === 'Rejected' && (
                      <div style={{ marginTop: 10, fontSize: 12, color: '#dc2626', background: '#fef2f2', borderRadius: 8, padding: '6px 10px' }}>
                        ❌ Rejected by admin — please contact your doctor
                      </div>
                    )}
                  </div>
                </div>
...
function DoctorPatients({ showToast, user }) {
  ...
  const loadPatients = useCallback(async () => {
    ...
    try {
      ...
      if (!doctor) return setLoading(false);

      const { data: prescriptions, error } = await supabase
        .from('prescription')
        .select('users(user_id,name,phone,age,gender,email)')
        .eq('doctor_id', doctor.doctor_id);

      if (error) showToast(error.message, 'error');
      else {
        const pts = prescriptions.map(p => p.users).filter(Boolean);
        const uniquePatients = [...new Map(pts.map(p => [p.user_id, p])).values()];
  ...
  return (
    <div className="section-card">
      ...
      {loading ? (
        ...
        <Empty icon="👥" text="No patients yet" />
      ) : (
        <table>
          <thead>
            <tr>
              <th>ID</th><th>Name</th><th>Phone</th><th>Age</th><th>Gender</th><th>Email</th>
            </tr>
          </thead>
          <tbody>
            {patients.map(u => (
              <tr key={u.user_id}>
                ...
                <td>{u.age || '—'}</td>
                <td>
                  <span className={`badge ${u.gender === 'Male' ? 'badge-blue' : 'badge-purple'}`}>
                    {u.gender || '—'}
                  </span>
                </td>
                <td>{u.email}</td>
...
function DoctorPrescriptions({ showToast, user }) {
  ...
  const [form, setForm] = useState({ user_id: '', date_issued: new Date().toISOString().split('T')[0], notes: '' });
  const [doctorId, setDoctorId] = useState(null);

  const loadPrescriptions = useCallback(async () => {
    setLoading(true);
    try {
      const { data: doctor } = await supabase.from('doctor').select('doctor_id').eq('email', user.email).single();
      if (!doctor) return;
      ...
      const { data, error } = await supabase
        .from('prescription')
        .select('prescription_id,user_id,date_issued,notes,users(name)')
        .eq('doctor_id', doctor.doctor_id)
  ...
  return (
    <div className="section-card">
      ...
      {loading ? (
        ...
        <Empty icon="📋" text="No prescriptions" />
      ) : (
        <table>
          <thead>
            <tr>
              <th>ID</th><th>Patient</th><th>Date</th><th>Notes</th><th>Status</th>
            </tr>
          </thead>
          <tbody>
            {prescriptions.map(p => (
      ...
      {modalOpen && (
        <Modal title="➕ Add Prescription" onClose={() => setModalOpen(false)}>
          <div className="form-group">
          ...
          <div className="modal-footer">
            ...
            <button className="btn btn-primary" onClick={savePrescription}>Add</button>
          </div>
        </Modal>
      )}
    </div>
...
function AdminDashboard({ showToast }) {
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      ...
      const { data: sc } = await supabase
        .from('schedule')
        .select('schedule_id,time,dosage(amount,unit,frequency,medicine(name))')
        .lte('start_date', today)
        .gte('end_date', today)
        .limit(10);
      setScheds(sc || []);

      // Fetch recent missed doses
  ...
  const statCards = [
    { label: 'Total Users', value: stats.users, accent: '#2563eb', accentLight: '#eff6ff' },
    { label: 'Doctors', value: stats.doctors, accent: '#0891b2', accentLight: '#ecfeff' },
    { label: 'Prescriptions', value: stats.prescriptions, accent: '#d97706', accentLight: '#fffbeb' },
    { label: 'Medicines', value: stats.medicines, accent: '#dc2626', accentLight: '#fef2f2' },
  ];
  ...
  return (
    <div>
      <div className="welcome-banner">
        <div>
          <div className="welcome-title">👑 Admin Dashboard</div>
      ...
      <div className="dash-grid">
        <div className="section-card">
          ...
          {loading ? (
            ...
            <Empty icon="📭" text="No schedules for today" />
          ) : (
            scheds.map(s => (
              <div key={s.schedule_id} className="schedule-item">
                ...
                <div className="schedule-info">
                  ...
                  <div className="schedule-detail">
                    {s.dosage?.amount} {s.dosage?.unit} · {s.dosage?.frequency}
                  </div>
                </div>
              </div>
            ))
        ...
        <div className="section-card">
          ...
          {loading ? (
            ...
            <Empty icon="✅" text="No missed doses!" />
          ) : (
            <table>
              <thead>
                <tr>
...
function AdminUsers({ showToast }) {
  ...
  const loadUsers = useCallback(async () => {
    ...
    try {
      ...
      if (usersError) return showToast(usersError.message, 'error');
      ...
      <div className="section-card">
        <div className="section-header">
          <div>
            ...
            <div className="section-subtitle">{data.length} total — patients, doctors & admins</div>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button
              ...
              onClick={() => { setForm({ name: '', phone: '', age: '', gender: 'Male', email: '' }); setModal('add'); }}
            >
              + Add User
            </button>
          </div>
        ...
        {loading ? (
          ...
          <Empty icon="👥" text="No users" />
        ) : (
          <table>
            <thead>
              <tr>
                <th>ID</th><th>Name</th><th>Phone</th><th>Age</th><th>Gender</th><th>Email</th><th>Role</th><th>Actions</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((u, i) => (
        ...
        <Modal title={modal === 'add' ? '➕ Add User' : '✏️ Edit User'} onClose={() => setModal(null)}>
          ...
          <div className="form-row">
            ...
            <div className="form-group"><label className="form-label">Gender</label>
              <select className="form-input" value={form.gender || 'Male'} onChange={e => setForm({ ...form, gender: e.target.value })}>
          ...
          <div className="modal-footer">
            ...
            <button className="btn btn-primary" onClick={saveUser}>{modal === 'add' ? 'Add' : 'Save'}</button>
          </div>
        </Modal>
      )}
    </div>
...
function AdminDoctors({ showToast }) {
  ...
  return (
    <div>
      <div className="section-card">
        <div className="section-header">
          <div><div className="section-title">🩺 All Doctors</div></div>
              ...
              <div className="doctor-grid">
                {data.map((d,i) => (
                  <div key={d.doctor_id} className="doctor-card">
                    <div className="doctor-card-accent" style={{ background:getDoctorAccent(i) }}></div>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginTop:8 }}>
                      <div className="doctor-avatar" style={{ background:`linear-gradient(135deg, ${getDoctorAccent(i)}33, ${getDoctorAccent(i)}55)`, color:getDoctorAccent(i) }}>
                        {getInitials(d.name)}
                      </div>
                      <div style={{ display:'flex', gap:6 }}>
                        <button className="btn btn-danger btn-sm" onClick={() => remove(d.doctor_id)}>🗑️</button>
                      </div>
                    </div>
...
function AdminPrescriptions({ showToast }) {
  ...
  return (
    <div>
      <div className="section-card">
        <div className="section-header">
          <div><div className="section-title">📋 All Prescriptions</div></div>
        ...
        {loading
          ...
          : data.length===0
            ...
            : (
              <table>
                <thead>
                  <tr><th>ID</th><th>Patient</th><th>Doctor</th><th>Date</th><th>Notes</th><th>Del</th></tr>
                </thead>
                <tbody>
                  {data.map(p => (
                    <tr key={p.prescription_id}>
        ...
        <Modal title="➕ Add Medicine" onClose={() => setModal(false)}>
          ...
          <div className="form-row">
            ...
            <div className="form-group">
              ...
              <input className="form-input" type="number" value={form.prescription_id} onChange={e => setForm({...form,prescription_id:e.target.value})}/>
            </div>
          </div>
          <div className="form-group">
...
function AdminDosages({ showToast }) {
  ...
  const save = async () => {
    const { error } = await supabase.from('dosage').insert({
  ...
  return (
    <div>
      <div className="section-card">
        <div className="section-header">
          <div>
            <div className="section-title">💉 All Dosages</div>
            <div className="section-subtitle">Manage medicine dosages</div>
          </div>
          <button className="btn btn-primary btn-sm" onClick={() => setModal(true)}>+ Add Dosage</button>
        </div>
        {loading ? <Loader/> : data.length === 0 ? <Empty icon="💉" text="No dosages yet"/> : (
          <table>
            <thead>
              <tr>
                <th>ID</th><th>Medicine</th><th>Amount</th><th>Unit</th><th>Frequency</th><th>Del</th>
              </tr>
            </thead>
            <tbody>
              {data.map(d => (
        ...
        <Modal title="➕ Add Dosage" onClose={() => setModal(false)}>
          ...
          <div className="form-row">
            <div className="form-group">
              ...
              <input className="form-input" type="number" placeholder="1" value={form.amount} onChange={e => setForm({...form, amount: e.target.value})}/>
            </div>
            <div className="form-group">
          ...
          <div className="modal-footer">
            <button className="btn btn-ghost" onClick={() => setModal(false)}>Cancel</button>
            <button className="btn btn-primary" onClick={save}>Add Dosage</button>
          </div>
        </Modal>
      )}
    </div>
...
function AdminSchedules({ showToast }) {
  ...
  const save = async () => {
    ...
    const { error } = await supabase.from('schedule').insert({
      dosage_id: parseInt(form.dosage_id),
  ...
  return (
    <div>
      <div className="section-card">
        <div className="section-header">
          <div>
            <div className="section-title">📅 All Schedules</div>
            <div className="section-subtitle">Manage medicine schedules</div>
          </div>
          <button className="btn btn-primary btn-sm" onClick={() => setModal(true)}>+ Add Schedule</button>
        </div>
        {loading ? <Loader/> : data.length === 0 ? <Empty icon="📅" text="No schedules"/> : (
          <table>
            <thead>
              <tr>
                <th>ID</th><th>Medicine</th><th>Dosage</th><th>Freq</th><th>Start</th><th>End</th><th>Time</th><th>Status</th><th>Del</th>
              </tr>
            </thead>
            <tbody>
              {data.map(s => {
        ...
        <Modal title="➕ Add Schedule" onClose={() => setModal(false)}>
          <div className="form-group">
            ...
            <select className="form-input" value={form.dosage_id} onChange={e => setForm({...form, dosage_id: e.target.value})}>
              ...
              {dosages.map(d => {
                const med = medicines.find(m => m.medicine_id === d.medicine_id)
                return <option key={d.dosage_id} value={d.dosage_id}>{med?.name} — {d.amount} {d.unit} ({d.frequency})</option>
              })}
            </select>
          </div>
          <div className="form-row">
          ...
          <div className="modal-footer">
            <button className="btn btn-ghost" onClick={() => setModal(false)}>Cancel</button>
            <button className="btn btn-primary" onClick={save}>Add Schedule</button>
          </div>
        </Modal>
      )}
    </div>
...
function AdminReminders({ showToast }) {
  ...
  return (
    <div className="section-card">
      <div className="section-header">
        <div>
          <div className="section-title">🔔 All Reminders</div>
        </div>
      </div>
      {loading ? <Loader /> :
        data.length === 0 ? <Empty icon="🔔" text="No reminders" /> :
        data.map(r => (
          <div key={r.reminder_id} className="reminder-card">
            <div className="reminder-icon-wrap">{icons[r.mode] || '🔔'}</div>
            <div className="reminder-info">
...
function AdminIntakeLogs({ showToast }) {
  ...
  return (
    <div className="section-card">
      <div className="section-header">
        <div>
          <div className="section-title">📝 All Intake Logs</div>
        </div>
        <button className="btn btn-ghost btn-sm" onClick={load}>↻</button>
      </div>
      {loading ? <Loader /> :
        data.length === 0 ? <Empty icon="📝" text="No logs" /> :
        <table>
          <thead>
            <tr>
              <th>Log ID</th><th>Schedule</th><th>Date</th><th>Time</th><th>Status</th><th>Del</th>
            </tr>
          </thead>
          <tbody>
            {data.map(l => (
