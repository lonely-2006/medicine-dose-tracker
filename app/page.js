...
function AuthPage({ onLogin }) {
  ...
  const handleRegister = async () => {
    ...
    if (error) setError(error.message)
    else setSuccess('Account created! Please check your email to verify, then login.')
    setLoading(false)
  }
    ...
    <div style={{
      ...
      <div style={{ textAlign: 'center', marginBottom: 8 }}>
        ...
        <div style={{ fontSize: 13, color: 'var(--muted)', marginTop: 4 }}>
          Your personal medicine management system
        </div>
      </div>

      <div style={{
        background: 'var(--card)', border: '1px solid var(--border)',
        borderRadius: 20, padding: 36, width: 420, maxWidth: '95vw'
      }}>
        {/* Tabs */}
        ...
        {error && (
          <div style={{ background: 'rgba(255,107,107,0.1)', border: '1px solid rgba(255,107,107,0.3)', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: 'var(--red)', marginBottom: 16 }}>
            ❌ {error}
          </div>
        )}
        {success && (
          <div style={{ background: 'rgba(0,212,170,0.1)', border: '1px solid rgba(0,212,170,0.3)', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: 'var(--teal)', marginBottom: 16 }}>
            ✅ {success}
          </div>
        )}
    ...
    <div>
      <div style={{
        ...
        <div>
          ...
          <div style={{ fontSize: 13, color: 'var(--muted)', marginTop: 4 }}>Here's your medicine tracker overview</div>
        </div>
      ...
      <div className="dash-grid">
        <div className="section-card">
          <div className="section-header">
            <div><div className="section-title">📅 Today's Schedule</div><div className="section-subtitle">Medicines due today</div></div>
        ...
        <div className="section-card">
          <div className="section-header">
            <div><div className="section-title">⚠️ Missed Doses</div><div className="section-subtitle">Recent missed intakes</div></div>
          </div>
...
function Users({ showToast }) {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const { data: d, error } = await supabase.from('users').select('user_id, name, phone, age, gender, email').order('user_id', { ascending: true })
  ...
  return (
    <div>
      <div className="section-card">
        <div className="section-header">
          <div><div className="section-title">👥 Users</div><div className="section-subtitle">All registered patients</div></div>
      ...
      {modal && (
        <Modal title={modal === 'add' ? '➕ Add New User' : '✏️ Edit User'} onClose={() => setModal(null)}>
          <div className="form-row">
            ...
            <div className="form-group"><label className="form-label">Phone</label><input className="form-input" value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} placeholder="9876543201" /></div>
          </div>
          <div className="form-row">
          ...
          <div className="form-group"><label className="form-label">Gmail</label><input className="form-input" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} placeholder="arjun@gmail.com" /></div>
...
function Doctors({ showToast }) {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const { data: d, error } = await supabase.from('doctor').select('doctor_id, name, specialization, email, phone').order('doctor_id', { ascending: true })
    if (error) showToast(error.message, 'error'); else setData(d)
    setLoading(false)
  }, [showToast])
  ...
  return (
    <div>
      <div className="section-card">
        <div className="section-header">
          <div><div className="section-title">🩺 Doctors</div><div className="section-subtitle">All registered doctors</div></div>
      ...
      {modal && (
        <Modal title="➕ Add New Doctor" onClose={() => setModal(false)}>
          ...
          <div className="form-row">
            <div className="form-group"><label className="form-label">Gmail</label><input className="form-input" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} placeholder="anil@gmail.com" /></div>
            <div className="form-group"><label className="form-label">Phone</label><input className="form-input" value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} placeholder="9111111101" /></div>
          </div>
          <div className="modal-footer">
...
function Prescriptions({ showToast }) {
  ...
  return (
    <div>
      <div className="section-card">
        <div className="section-header">
          <div><div className="section-title">📋 Prescriptions</div><div className="section-subtitle">All prescriptions issued</div></div>
          ...
          <table>
            ...
            <tbody>{data.map(p => (
              <tr key={p.prescription_id}>
                <td>#{p.prescription_id}</td><td><strong>{p.users?.name || '—'}</strong></td><td>{p.doctor?.name || '—'}</td>
                <td>{p.date_issued}</td>
...
function Medicines({ showToast }) {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const { data: d, error } = await supabase.from('medicine').select('medicine_id, name, brand, type, description, prescription_id').order('medicine_id', { ascending: true })
    if (error) showToast(error.message, 'error'); else setData(d)
    setLoading(false)
  }, [showToast])
  ...
  return (
    <div>
      <div className="section-card">
        <div className="section-header">
          <div><div className="section-title">💉 Medicines</div><div className="section-subtitle">All medicines in the system</div></div>
      ...
      {modal && (
        <Modal title="➕ Add Medicine" onClose={() => setModal(false)}>
          ...
          <div className="form-row">
            ...
            <div className="form-group"><label className="form-label">Prescription ID</label><input className="form-input" type="number" value={form.prescription_id} onChange={e => setForm({ ...form, prescription_id: e.target.value })} placeholder="1" /></div>
          </div>
          <div className="form-group"><label className="form-label">Description</label><input className="form-input" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} placeholder="Controls blood sugar" /></div>
          <div className="modal-footer">
            <button className="btn btn-ghost" onClick={() => setModal(false)}>Cancel</button>
...
function Schedules({ showToast }) {
  ...
  const load = useCallback(async () => {
    ...
    const { data: d, error } = await supabase.from('schedule')
      .select('schedule_id, start_date, end_date, time, dosage_id, dosage(amount, unit, frequency, medicine(name))').order('schedule_id', { ascending: true })
    if (error) showToast(error.message, 'error'); else setData(d)
    setLoading(false)
  }, [showToast])
  ...
  return (
    <div className="section-card">
      <div className="section-header">
        <div><div className="section-title">📅 Schedules</div><div className="section-subtitle">All medicine schedules</div></div>
        <button className="btn btn-ghost btn-sm" onClick={load}>↻ Refresh</button>
      </div>
...
function Reminders({ showToast }) {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const { data: d, error } = await supabase.from('reminder').select('reminder_id, schedule_id, mode, reminder_time, status').order('reminder_id', { ascending: true })
    if (error) showToast(error.message, 'error'); else setData(d)
    setLoading(false)
  }, [showToast])
  ...
  return (
    <div className="section-card">
      <div className="section-header"><div><div className="section-title">🔔 Reminders</div><div className="section-subtitle">Manage all reminders</div></div></div>
...
function IntakeLogs({ showToast }) {
  ...
  return (
    <div>
      <div className="section-card">
        <div className="section-header">
          <div><div className="section-title">📝 Intake Log</div><div className="section-subtitle">All dose intake records</div></div>
