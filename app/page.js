...
function DoctorPrescriptions({ showToast, user }) {
  ...
  useEffect(() => {
    supabase.from('users').select('user_id,name,email').eq('role','patient').then(({ data }) => setPatients(data||[]))
  }, [])
...
function AdminUsers({ showToast }) {
  ...
  const load = useCallback(async () => {
    ...
    const patientRows = (users||[]).map(u => ({ ...u, _type: u.role || (u.is_admin?'admin': u.is_doctor?'doctor':'patient') }))
...
function AdminPrescriptions({ showToast }) {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data:d, error }, { data:pts }, { data:drs }] = await Promise.all([
      ...
      supabase.from('users').select('user_id,name,email').eq('role','patient'),
